...
if(-e $file){
	...
	for($p = 0; $p < @{$rtn->{'pages'}}; $p++){
		if($p == 0){
			...
			# Get the page
			$url = "https://www.newcastle-staffs.gov.uk".$rtn->{'pages'}[$p];
			if($rtn->{'pages'}[$p] =~ /a_to_z\/([A-Z])/){
				$rfile = "raw/newcastle_under_lyme_council-$1.html";
			}else{
				$rfile = "raw/newcastle_under_lyme_council-$p.html";
			}
		...
		for($i = 0; $i < @{$res->{'warmspaces'}}; $i++){
			...
			$d = $res->{'warmspaces'}[$i];
			if($d->{'url'} =~ /^\//){ $d->{'url'} = "https://www.newcastle-staffs.gov.uk".$d->{'url'}; }

			$d->{'url'} =~ /directory-record\/([^\/]*)\//;
			$record = $1;
			$rfile = "raw/newcastle_under_lyme_council-$record.html";
			...
			while($html =~ s/<dt[^\>]*>(.*?)<\/dt>[\n\r\t\s]+<dd[^>]*>(.*?)<\/dd>//s){
				...
				}elsif($key =~ /Contact information/is && $value =~ /href="([^\"]+)"/){
					$d->{'url'} = $1;
				}elsif($key =~ /Location/is && $value =~ /id="map_marker_location.* value="([^\"]+),([^\"]+)"/){
					$d->{'lat'} = $1;
					$d->{'lon'} = $2;
